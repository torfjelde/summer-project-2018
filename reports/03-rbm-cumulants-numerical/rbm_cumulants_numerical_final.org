...
Suppose we know that a series converges, but not necessarily /absolutely/, and we do not know the pattern of the change of signs in the terms of the series. If we thought it reasonable to approximate the series using a truncation of the series to the first, say $N$, terms, we would /at the very least/ have to observe the magnitude of the terms decreasing for all $N_0 \le n \le N$, for some $N_0 < N$. Otherwise, there would be no reason for us to believe that the trunacted series was in any way a /good/ approximation.
